/*
 * Adjusted copy of mwdisk-declining-vcirc-disptens-rsunfree-model.stan in order to fit the model specified in rotcurve-bp.py.
 *
 *
 ...
 *  Rsun: Distance from sun to Galactic centre (kpc)
...
functions{
  array[] vector predicted_proper_motions(vector plx, array[] vector p, array[] vector q, array[] vector r,
    ...
    real Rstar;
    vector[3] rotcur;  // Added
    vector[3] v0;  // Added
    real h; // Added
    //real p; Already in use. p[i] threevector of star i

    for (i in 1:size(plx)) {
      starpos = (1000.0/plx[i])*r[i] + sunpos;
      Rstar = sqrt(starpos[1]^2+starpos[2]^2);
      rotcur = Rstar/h * (1 + (Rstar/h)^2)^((p[i]-2)/4);  // Added, rhs is a vector
      v0 = Vcirc_sun / rotcur;  // Added
      vphistar = v0*( (1/h) * (1 + (Rstar/h)^2)^((p[i]-2)/4) + (Rstar^2/h^3)*((p[i]-2)/2)* (1 + (Rstar/h)^2)^((p[i]-6)/4) );  // Adjusted from original stanfile
      phi = atan2(starpos[2], starpos[1]);
      vdiff = [-vphistar*sin(phi), vphistar*cos(phi), 0.0]' - vsun;
      predicted_pm[i][1] = dot_product(p[i], vdiff) * plx[i] / Av;  // For pml
      predicted_pm[i][2] = dot_product(q[i], vdiff) * plx[i] / Av;  // For pmb
    }
...
transformed data {
  ...
  real Rsun_prior_mean = 8.0;
  real Rsun_prior_sigma = 0.1;
...
parameters {
  ...
  real<lower=0> Rsun;          // Distance from Sun to Galactic centre
...
transformed parameters {
  ...
  model_pm = predicted_proper_motions(plx_obs, pvec, qvec, rvec, auKmYearPerSec, [-1000.0*Rsun, Ysun, Zsun]',
...
model {
  ...
  Rsun ~ normal(Rsun_prior_mean, Rsun_prior_sigma);
