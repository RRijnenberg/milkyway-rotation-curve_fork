...
functions{
  array[] vector predicted_proper_motions(vector plx, array[] vector p, array[] vector q, array[] vector r,
      real Av, vector sunpos, vector Vsun_pec, real Vcirc_sun, real dVcirc_dVr, real h_param, real p_param) {
    /*
    ...
    for (i in 1:size(plx)) {
      ...
      vdiff = [-vphistar*sin(phi), vphistar*cos(phi), 0.0]' - vsun; // is vsun hier nog nodig???
      predicted_pm[i][1] = dot_product(p[i], vdiff) * plx[i] / Av;
      predicted_pm[i][2] = dot_product(q[i], vdiff) * plx[i] / Av;
    } \\ Vsun voor elke i berekenen, zit impliciet in het model
...
transformed data {
  ...
  real Vcirc_sun_prior_mean = 220.0; // Onnodig
  real Vcirc_sun_prior_sigma = 50.0; // Onnodig
  real dVcirc_dVr_prior_mean = 0.0; // Onnodig
  real dVcirc_dVr_prior_sigma = 10.0; // Onnodig
  real Vsun_pec_x_prior_mean = 11.0; // Onnodig
  real Vsun_pec_y_prior_mean = 12.0; // Onnodig
  real Vsun_pec_z_prior_mean = 7.0; // Onnodig
  real Vsun_pec_x_prior_sigma = 20.0; // Onnodig
  real Vsun_pec_y_prior_sigma = 20.0; // Onnodig
  real Vsun_pec_z_prior_sigma = 20.0; // Onnodig
  real vdisp_prior_alpha = 2.0;
  real vdisp_prior_beta = 0.1;
  real h_param_prior_alpha = 2.0;  // Aanpassen! Wel goed wsl
  real h_param_prior_beta = 1.0;  // Aanpassen! Goed genoeg wsl (breedte nog iets aanpassen --> groter bereik)
  real p_param_mean = -0.5;  // Aanpassen!
  real p_param_sigma = 1.5;  // Aanpassen!
...
parameters {
  ...
  real<lower=0> h_param;  // Added
  real p_param;  // Added Kan zonder constraint
}
...
transformed parameters {
  ...
  model_pm = predicted_proper_motions(plx_obs, pvec, qvec, rvec, auKmYearPerSec, [-Rsun, Ysun, Zsun]',
    [Vsun_pec_x, Vsun_pec_y, Vsun_pec_z]', Vcirc_sun, dVcirc_dVr, h_param, p_param);
}
...
model {
  ...
  p_param  ~ normal(p_param_mean, p_param_sigma);  // p_param ~ [-2, 1]
