...
transformed data {
  ...
  real amplitude_r_prior_mean = 80;
  real amplitude_r_prior_sigma = 20;
  real amplitude_phi_prior_mean = 80;
  real amplitude_phi_prior_sigma = 20;
  real amplitude_z_prior_mean = 80;
  real amplitude_z_prior_sigma = 20;
  real R_scale_r_prior_low = 0.25;
  real R_scale_r_prior_high = 30;
  real R_scale_phi_prior_low = 0.25;
  real R_scale_phi_prior_high = 30;
  real R_scale_z_prior_low = 0.25;
  real R_scale_z_prior_high = 30;
  ...
  for (n in 1:N) {
    ...
    J[n][1,3] = 0.0;    // J = conversion matrix to adjust covar(R, phi, Z) to covar(x, y, z)
    J[n][2,3] = 0.0;
    J[n][3,1] = 0.0;
    J[n][3,2] = 0.0;
    J[n][3,3] = 1.0;
...
parameters {
  ...
  real<lower=0> amplitude_r;    // Amplitude of the velocity dispersion around circular motion in R direction
  real<lower=0> amplitude_phi;  // Amplitude of the velocity dispersion around circular motion in Phi direction
  real<lower=0> amplitude_z;    // Amplitude of the velocity dispersion around circular motion in Z direction
  real<lower=0> R_scale_r;      // Scalelength of the velocity dispersion around circular motion in R direction
  real<lower=0> R_scale_phi;    // Scalelength of the velocity dispersion around circular motion in Phi direction
  real<lower=0> R_scale_z;      // Scalelength of the velocity dispersion around circular motion in Z direction
...
transformed parameters {
  ...
  for (n in 1:N) {
    // Added vdisp behaviour of giants:
    if (Rstar[n] < 2.5) {
      covar_rpz[n][1,1] = amplitude_r;
      covar_rpz[n][2,2] = amplitude_phi;
    }
    else {
      covar_rpz[n][1,1] = amplitude_r * exp(-(Rstar[n]-2.5)/R_scale_r);
      covar_rpz[n][2,2] = amplitude_phi * exp(-(Rstar[n]-2.5)/R_scale_phi);
    }
    covar_rpz[n][3,3] = amplitude_z * exp(-Rstar[n]/R_scale_z);

    scov_cyl = diag_matrix([covar_rpz[n][1,1]^2, covar_rpz[n][2,2]^2, covar_rpz[n][3,3]^2]');
...
model {
  ...
  amplitude_r ~ normal(amplitude_r_prior_mean, amplitude_r_prior_sigma);
  amplitude_phi ~ normal(amplitude_phi_prior_mean, amplitude_phi_prior_sigma);
  amplitude_z ~ normal(amplitude_z_prior_mean, amplitude_z_prior_sigma);
  R_scale_r ~ uniform(R_scale_r_prior_low, R_scale_r_prior_high);
  R_scale_phi ~ uniform(R_scale_phi_prior_low, R_scale_phi_prior_high);
  R_scale_z ~ uniform(R_scale_z_prior_low, R_scale_z_prior_high);
  for (i in 1:N) {
