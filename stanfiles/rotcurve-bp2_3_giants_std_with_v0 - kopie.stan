...
functions{
  ...
  real xy_disp_giants(real plx, vector r, vector sunpos, real amplitude, real R_scale, real v0) {
	/* sigma(R)
  ...
  */
  ...
  if (Rstar < 2500) {
    ...
    disp_mean = amplitude * exp(-Rstar/R_scale);
  }
  return disp_mean;
  }
...
transformed parameters {
  ...
  for (n in 1:N) {
    scov[1,1] = (xy_disp_giants(plx_obs[n], rvec[n], [-Rsun, Ysun, Zsun]', amplitude_x, 1000*R_scale_x))^2;
    scov[2,2] = (xy_disp_giants(plx_obs[n], rvec[n], [-Rsun, Ysun, Zsun]', amplitude_y, 1000*R_scale_y))^2;
    scov[3,3] = (z_disp_giants(plx_obs[n], rvec[n], [-Rsun, Ysun, Zsun]', amplitude_z, 1000*R_scale_z))^2;
